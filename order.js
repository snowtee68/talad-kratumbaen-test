(() => {
  ...
  const MAX_PICKUPS=5, MAX_ROUTE_KM=10, EXTRA_PICKUP_FEE=10;
...
if(e.target.closest('#showDeliveryFareInfoBtn'))return showDeliveryFareInfo(false);if(e.target.closest('#closeDeliveryFareInfoBtn'))return closeModal();
    ...
    document.addEventListener('click',e=>{
      ...
      if(e.target.closest('#chooseDeliveryMapBtn'))return openDeliveryMapPicker();
      if(e.target.closest('#deliveryMapConfirmBtn'))return confirmDeliveryMapPicker();
      if(e.target.closest('#deliveryMapCloseBtn')||e.target.closest('#deliveryMapBackdrop'))return closeDeliveryMapPicker();
  ...
  function captureDeliveryLocation(){if(!navigator.geolocation)return alert('อุปกรณ์นี้ไม่รองรับตำแหน่ง');const box=document.getElementById('deliveryLocationStatus');if(box)box.textContent='⏳ กำลังระบุตำแหน่ง...';navigator.geolocation.getCurrentPosition(p=>{document.getElementById('coLat').value=p.coords.latitude.toFixed(7);document.getElementById('coLng').value=p.coords.longitude.toFixed(7);updateDeliveryLocationStatus();refreshCheckoutFarePreview();},e=>{updateDeliveryLocationStatus();if(window.KBLocationHelp)window.KBLocationHelp.show({error:e,retry:captureDeliveryLocation,context:'การจัดส่งสินค้า'});else alert('ระบุตำแหน่งไม่สำเร็จ: '+e.message)}, {enableHighAccuracy:true,timeout:12000,maximumAge:15000});}
  let deliveryPickerMap=null,deliveryPickerMarker=null;
  function openDeliveryMapPicker(){
    if(typeof L==='undefined')return alert('ไม่สามารถเปิดแผนที่ได้ กรุณาลองใหม่');
    closeDeliveryMapPicker();
    const currentLat=Number(document.getElementById('coLat')?.value),currentLng=Number(document.getElementById('coLng')?.value);
    const hasCurrent=Number.isFinite(currentLat)&&Number.isFinite(currentLng)&&currentLat!==0&&currentLng!==0;
    const lat=hasCurrent?currentLat:13.6536,lng=hasCurrent?currentLng:100.2590;
    const wrap=document.createElement('div');wrap.id='deliveryMapPicker';wrap.className='delivery-map-picker';
    wrap.innerHTML=`<div id="deliveryMapBackdrop" class="delivery-map-backdrop"></div><div class="delivery-map-panel"><div class="delivery-map-head"><div><b>🗺️ เลือกจุดจัดส่ง</b><div>แตะบนแผนที่หรือลากหมุดไปยังตำแหน่งที่ Rider ต้องไปส่ง</div></div><button type="button" id="deliveryMapCloseBtn" aria-label="ปิด">×</button></div><div id="deliveryPickerMap" class="delivery-picker-map"></div><div class="delivery-map-note">กรุณาตรวจตำแหน่งหมุดให้ตรงกับบ้าน/จุดรับสินค้า ก่อนยืนยัน</div><button type="button" id="deliveryMapConfirmBtn" class="mo-primary delivery-map-confirm">✓ ใช้ตำแหน่งนี้</button></div>`;
    ...
    deliveryPickerMarker=L.marker([lat,lng],{draggable:true}).addTo(deliveryPickerMap);
    deliveryPickerMap.on('click',e=>deliveryPickerMarker.setLatLng(e.latlng));
    setTimeout(()=>deliveryPickerMap?.invalidateSize(),80);
  }
  function confirmDeliveryMapPicker(){
    if(!deliveryPickerMarker)return;const p=deliveryPickerMarker.getLatLng();
    const lat=document.getElementById('coLat'),lng=document.getElementById('coLng');if(!lat||!lng)return closeDeliveryMapPicker();
    lat.value=Number(p.lat).toFixed(7);lng.value=Number(p.lng).toFixed(7);
    closeDeliveryMapPicker();updateDeliveryLocationStatus();refreshCheckoutFarePreview();
  }
